...
import com.gs2e.datacollectoriot.config.EmbeddedKafka;
import com.gs2e.datacollectoriot.config.KafkaSseConsumer;
import com.gs2e.datacollectoriot.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class DataCollectorIoTKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/data-collector-io-t-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/data-collector-io-t-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/data-collector-io-t-kafka/unregister"));
                return;
            }
        }
